...
namespace Script.Main
{
    ...
    public class BlinkStar : MonoBehaviour, IClickables
    {
        ...
        private void Awake()
        {
            ...
            _firstQuest = _dataDic.FirstQuestsOfScene.First();
            _lastQuest = _dataDic.LastQuestsOfScene.Last();
                    ...
                    else
                    {
                        ...
                        foreach (var term in _ownQuest.TermsDic)
                        {
                            ...
                            if (termItemIndex == 9999) // 조건이 골드일 때
                                ...
                                GameObject.Find("Progress Displayer").GetComponent<Text>().text += isTermFirst
                                    ? string.Format("{0} {1} / {2}", _dataDic.FindUpgrade(termItemIndex).Name,
                                        UpgradeManager.GetUpgradeLv(termItemIndex), term.Value)
                                    : string.Format("\n{0} {1} / {2}", _dataDic.FindUpgrade(termItemIndex).Name,
                                        UpgradeManager.GetUpgradeLv(termItemIndex), term.Value);
                            else
                                GameObject.Find("Progress Displayer").GetComponent<Text>().text += isTermFirst
                                    ? string.Format("{0} {1} / {2}", _dataDic.FindItemDic[termItemIndex].Name,
                                        _dataController.GetItemNum(termItemIndex), term.Value)
                                    : string.Format("\n{0} {1} / {2}", _dataDic.FindItemDic[termItemIndex].Name,
                                        _dataController.GetItemNum(termItemIndex), term.Value);
            ...
            foreach (var reward in _ownQuest.RewardDic)
            {
                ...
                if (rewardIndex == 9999)
                    GameObject.Find("Reward Displayer").GetComponent<Text>().text += isRewardFirst
                        ? string.Format("골드 {0}", reward.Value)
                        : string.Format("\n골드 {0}", reward.Value);
        ...
        private static int _firstQuest;
        private static int _lastQuest; // 원래는 90247
        ...
        public static int ShowingQuestIndex
        {
            ...
            private set
            {
                if (value < _firstQuest)
                    _showingQuestIndex = _firstQuest;
                else if (value > _lastQuest)
                    _showingQuestIndex = _lastQuest;
                else
